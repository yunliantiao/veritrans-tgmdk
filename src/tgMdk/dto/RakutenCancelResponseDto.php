...
/**
 * 決済サービスタイプ：楽天、コマンド名：取消の応答Dtoクラス<br>
 *
 * @author Veritrans, Inc.
 *
 */
class RakutenCancelResponseDto extends MdkBaseDto
{
     ...
     * 最大桁数：7<br/>
     * 現在の残高（返金後の売上残高）<br/>
     * ※バーコード決済（店舗スキャン型）、オンライン決済（V2）、バーコード決済（消費者スキャン型）で返却します。<br/>
     */
    private $balance;
    ...
    /**
     * 決済取消トランザクションIDを取得する<br>
     * @return string 決済取消トランザクションID<br>
     */
    public function getCancelTxnId()
    {
        return $this->cancelTxnId;
    }

    /**
     * 決済取消トランザクションIDを設定する<br>
     * @param string $cancelTxnId 決済取消トランザクションID<br>
     */
    public function setCancelTxnId($cancelTxnId)
    {
